...
void R_DrawAlphaSurfaces(void)
{
    ...
    while (s) {
        ...
        if (s->texinfo->c.flags & SURF_TRANS66)
            R_ClipAndDrawPoly(0.66f, (s->texinfo->c.flags & (SURF_WARP | SURF_FLOWING)), 1);
        else if (s->texinfo->c.flags & SURF_TRANS33)
            R_ClipAndDrawPoly(0.33f, (s->texinfo->c.flags & (SURF_WARP | SURF_FLOWING)), 1);
        else //qb: fence textures
            R_ClipAndDrawPoly(1.0f, (s->texinfo->c.flags & (SURF_WARP | SURF_FLOWING)), 2);
...
void R_IMFlatShadedQuad(vec3_t a, vec3_t b, vec3_t c, vec3_t d, color_t color, float alpha)
{
    ...
    R_ClipAndDrawPoly(alpha, qfalse, 0);
}
